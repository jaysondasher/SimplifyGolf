    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                VStack {
                    SearchBar(text: $viewModel.searchText, onCommit: viewModel.performSearch)
                        .padding()
        ...
        .fullScreenCover(isPresented: $viewModel.showingAddHoles) {
            if let selectedCourse = viewModel.selectedCourse {
                AddCourseMapView(course: selectedCourse, courseRating: viewModel.courseRating, slopeRating: viewModel.slopeRating)
            }
        }
    }
}
...
class AddNewCourseViewModel: NSObject, ObservableObject {
    @Published var centerCoordinate = CLLocationCoordinate2D(latitude: 37.3314, longitude: -122.0325)
    ...
    func performSearch() {
        ...
        search.start { [weak self] (response, error) in
            ...
            if let response = response {
                self.searchResults = response.mapItems.map { SearchResult(title: $0.name ?? "", subtitle: $0.placemark.title ?? "", mapItem: $0) }
