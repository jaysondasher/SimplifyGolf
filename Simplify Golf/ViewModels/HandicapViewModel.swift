...
class HandicapViewModel: ObservableObject {
    ...
    func fetchHandicapData() {
        ...
        db.collection("rounds")
            ...
            .getDocuments { [weak self] (querySnapshot, error) in
                ...
                let rounds = documents.compactMap {
                    document -> (id: String, score: Int, courseId: String)? in
                    if let scores = document.data()["scores"] as? [Int],
                        let courseId = document.data()["courseId"] as? String
                    {
                        // Filter out incomplete rounds
                        let playedHoles = scores.filter { $0 > 0 }
                        if playedHoles.count >= 18 {  // Only consider rounds with at least 18 played holes
                            let totalScore = playedHoles.reduce(0, +)
    ...
    private func fetchCourseData(for rounds: [(id: String, score: Int, courseId: String)]) {
        ...
        group.notify(queue: .main) { [weak self] in
            ...
            let completeRounds = rounds.compactMap {
                round -> (score: Int, rating: Double, slope: Int)? in
    ...
    private func calculateHandicap(rounds: [(score: Int, rating: Double, slope: Int)]) {
        ...
        let differentials = rounds.map {
            (113.0 / Double($0.slope)) * (Double($0.score) - $0.rating)
        }
        ...
        let averageDifferential =
            relevantDifferentials.reduce(0, +) / Double(relevantDifferentials.count)
        ...
        print(
            "Handicap index: \(String(describing: handicapIndex)), Recent scores: \(recentScores)")
    }
}
